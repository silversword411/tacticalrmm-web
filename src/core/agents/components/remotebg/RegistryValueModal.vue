    ...
    <q-card style="min-width: 420px; max-width: 480px">
      ...
      <q-card-actions align="right" class="q-gutter-sm">
        <q-btn
          ...
          @click="onSave"
        />
        <q-btn v-close-popup label="Cancel" flat :disable="isSaving" />
      </q-card-actions>
    </q-card>
  </q-dialog>
</template>
...
import type { RegistryValue } from "src/core/agents/types";
...
const stringValue = ref<string>(typeof props.row.data === "string" ? props.row.data : "");
...
function parseValue(data: string | number): number {
  ...
  if (typeof data === "string") {
    ...
    if (matchHex?.[0]) return parseInt(matchHex[0], 16);
    if (matchDec?.[1]) return parseInt(matchDec[1], 10);
  ...
  (newVal) => {
    ...
    if (newVal.type === "REG_DWORD" || newVal.type === "REG_QWORD") {
      numericValue.value = parseValue(
        typeof newVal.data === "string" || typeof newVal.data === "number" ? newVal.data : 0,
      );
      localBase.value = "hex";
    }
...
function onSave() {
  ...
  if (localRow.value.type === "REG_DWORD" || localRow.value.type === "REG_QWORD") {
    const maxValue =
