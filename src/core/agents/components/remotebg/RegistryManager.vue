<template>
  <div>
    <div class="text-body2 row items-center px-2 folder-path">
      <q-input v-model="pathInput" dense class="col" borderless @keyup.enter="navigateToPath">
        <template #prepend>
          <q-icon name="fa-solid fa-display" class="q-mr-sm text-blue-5" size="18px" />
        </template>
      </q-input>
    </div>
          ...
          <template #default-header="prop">
            ...
            <div v-else class="q-ml-xs q-my-xs">
              <q-btn
                ...
                color="primary"
                label="Load More"
                class="q-px-xs q-py-xs font-weight-medium"
                :loading="loadingMoreNodes[getParentIdFromLoadNode(prop.node.id)]"
            ...
            >
              <q-list dense style="min-width: 180px">
                ...
                <q-item v-close-popup clickable @click="refreshNode(prop.node.id)">
                  <q-item-section>Refresh</q-item-section>
                </q-item>
                <q-item v-close-popup clickable @click="renameKey(prop.node)">
                  <q-item-section>Rename</q-item-section>
                </q-item>
                <q-item v-close-popup clickable @click="deleteKey(prop.node)">
          ...
          <div class="col relative-position" style="flex: 1; overflow: auto">
            ...
            >
              <q-list dense style="min-width: 130px">
                <q-item clickable style="padding-right: 3px; padding-left: 12px">
                  <q-item-section>New</q-item-section>
                  <q-item-section side>
                    <q-icon name="chevron_right" />
                  </q-item-section>
            ...
            >
              <template #body-cell-name="cellProps">
                <q-td :props="cellProps" class="registry-cell">
                  <div class="registry-item">
                    <q-icon
                      :name="`img:${cellProps.row.type === 'REG_MULTI_SZ' || cellProps.row.type === 'REG_SZ' || cellProps.row.type === 'REG_EXPAND_SZ' ? stringTypeIcon : binaryTypeIcon}`"
                      size="14px"
                      class="registry-icon"
                    />
                    <q-input
                      v-if="editingValueName === cellProps.row.name"
                      ref="editInputRef"
                      ...
                      style="max-width: 200px"
                      @click.stop
                      @keyup.enter="finishRenameValue(cellProps.row, 'enter')"
                      @blur="finishRenameValue(cellProps.row, 'blur')"
                    />
                    <div v-else class="registry-name" @dblclick="openModifyDialog(cellProps.row)">
                      <div class="cell-text">{{ cellProps.row.name }}</div>
                      <div v-if="cellProps.row.name" class="cell-hover">
                        {{ cellProps.row.name }}
                      </div>
                    </div>
                  </div>
                  <q-menu context-menu transition-show="jump-up" transition-hide="jump-down">
                    <q-list dense style="min-width: 180px">
                      <q-item v-close-popup clickable @click="openModifyDialog(cellProps.row)">
                        <q-item-section>Modify</q-item-section>
                      </q-item>
                      <q-item v-close-popup clickable @click="renameValue(cellProps.row)">
                        <q-item-section>Rename</q-item-section>
                      </q-item>
                      <q-item v-close-popup clickable @click="deleteValue(cellProps.row)">
                        <q-item-section>Delete</q-item-section>
                      </q-item>
                    </q-list>
                  </q-menu>
              ...
              <template #body-cell-type="typeProps">
                <q-td :props="typeProps">{{ typeProps.row.type }}</q-td>
              </template>
              <template #body-cell-data="dataProps">
                <q-td :props="dataProps">
                  <span v-if="Array.isArray(dataProps.row.data)">
                    {{
                      JSON.stringify(dataProps.row.data)
                        .replace(/^\[|\]$/g, "")
                        .replace(/['"]/g, "")
                        .replace(/,/g, ", ")
                    }}
    ...
    <ConfirmDialog
      ...
      icon-color="orange"
    />
    <RegistryValueModal v-model="modifyDialog" :row="modifyRow" @save="saveModify" />
  </div>
</template>
...
import { registryTableColumns, registryValueTypes } from "src/constants/constants";
import type { RegistryNode, RegistryValue } from "src/core/agents/types";
import {
  createRegistryKey,
  createRegistryValue,
  deleteRegistryKey,
...
import RegistryValueModal from "./RegistryValueModal.vue";
import stringTypeIcon from "src/assets/string_type.png";
import binaryTypeIcon from "src/assets/binary_type.png";
...
const inFlightLoads: Record<string, Array<(children: RegistryNode[]) => void>> = {};
...
onMounted(async () => {
  ...
  try {
    ...
    const data = await fetchAgentRegistry(props.agentId, rootPath);
    const keys = data?.subkeys || [];
    registryNodes.value = keys.map((key: { name: string; hasSubkeys: boolean }) => ({
      id: `${key.name}\\`,
      label: key.name,
      lazy: key.hasSubkeys,
    }));
...
function toggleNode(node: RegistryNode) {
  ...
  registryTree.value.setExpanded(node.id, !registryTree.value.isExpanded(node.id));
}
...
async function loadChildren({
  ...
  try {
    ...
    const data = await fetchAgentRegistry(props.agentId, node.id, currentPage, searchKeys.value);
    const keys = data?.subkeys || [];
    const children: RegistryNode[] = keys.map((key: { name: string; hasSubkeys: boolean }) => {
      const id = node.id.endsWith("\\") ? `${node.id}${key.name}\\` : `${node.id}\\${key.name}\\`;
...
function onClickLoadMoreFromPlaceholder(placeholderNode: RegistryNode) {
  ...
  if (parent) void loadMoreSubkeys(parent);
}
...
async function loadMoreSubkeys(parent: RegistryNode) {
  ...
  try {
    ...
    const data = await fetchAgentRegistry(props.agentId, parentId, nextPage);
    const keys = data?.subkeys || [];
    const newChildren = keys.map((key: { name: string; hasSubkeys: boolean }) => {
    ...
    const filteredChildren = (parent.children ?? []).filter((c) => !c.isLoadMore);
...
async function onKeySelect(nodeId: string) {
  ...
  loading.value = true;
  try {
    const data = await fetchAgentRegistry(props.agentId, nodeId);
...
async function confirmDeleteKey() {
  ...
  loading.value = true;
  try {
    await deleteRegistryKey(props.agentId, nodeToDelete.value.id);
    const parentId = nodeToDelete.value.id.split("\\").slice(0, -2).join("\\") + "\\";
    const parent = findNodeById(registryNodes.value, parentId);
    if (parent?.children) {
      parent.children = parent.children.filter((child) => child.id !== nodeToDelete.value!.id);
      registryNodes.value = JSON.parse(JSON.stringify(registryNodes.value));
    }
    if (parentId) {
      selectedKey.value = parentId;
      await nextTick();
      void onKeySelect(parentId);
    }
  } catch (err) {
...
async function createKey(parentNode: RegistryNode) {
  try {
    const data = await fetchAgentRegistry(props.agentId, parentNode.id);
    const keys = data?.subkeys || [];
    parentNode.children = keys.map((key: { name: string; hasSubkeys: boolean }) => {
  ...
  if (!pendingNodes.value[parentNode.id]) pendingNodes.value[parentNode.id] = [];
...
function safeCreateKey() {
  ...
  if (parentNode) {
    void createKey(parentNode);
  } else {
    warningDialog.value = true;
  }
}

async function finishRename(node: RegistryNode, source: "enter" | "blur" = "enter") {
  ...
  try {
    loading.value = true;
    if (isTempNode) {
      await createRegistryKey(props.agentId, newId);
    } else if (originalId !== newId) {
      await renameRegistryKey(props.agentId, originalId, newId);
    }
    node.id = newId;
    node.label = newName;
...
function removeTempNode(node: RegistryNode) {
  ...
  if (pendingNodes.value[parentId]) {
    pendingNodes.value[parentId] = pendingNodes.value[parentId].filter((n) => n.id !== node.id);
  }
}
...
async function confirmDeleteValue() {
  ...
  loading.value = true;
  try {
    await deleteRegistryValue(props.agentId, currentPath.value, valueToDelete.value.name);
    tableRows.value = tableRows.value.filter((r) => r.name !== valueToDelete.value!.name);
  } catch (err) {
...
async function finishRenameValue(row: RegistryValue, source: "enter" | "blur" = "enter") {
  ...
  try {
    loading.value = true;
    await renameRegistryValue(props.agentId, currentPath.value, row.name, newName);
    const index = tableRows.value.findIndex((r) => r.name === row.name);
    if (index !== -1 && tableRows.value[index]) tableRows.value[index].name = newName;
  } catch (err) {
...
function createValue(node: RegistryNode | undefined, type: string, newKey: boolean = false) {
  const targetNode = node || findNodeById(registryNodes.value, selectedKey.value || "");
  if (!targetNode?.id) {
    warningDialog.value = true;
    return;
  }
  const defaultName = "";
  const defaultData =
    type === "REG_MULTI_SZ" ? [] : type === "REG_DWORD" || type === "REG_QWORD" ? 0 : "";
...
async function navigateToPath() {
  ...
  if (inputPath.startsWith("Computer")) inputPath = inputPath.replace(/^Computer[/\\]*/, "");
  loading.value = true;
  try {
    searchKeys.value = true;
    const data = await fetchAgentRegistry(props.agentId, inputPath, 1, searchKeys.value);
...
async function expandTreeToPath(path: string) {
  ...
  void scrollSelectedNodeIntoView();
}
