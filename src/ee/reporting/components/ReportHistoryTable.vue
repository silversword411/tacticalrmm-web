...
<template>
  <q-dialog ref="dialogRef" maximized @hide="onDialogHide">
    <q-card>
      <q-bar>
        <q-btn class="q-mr-sm" dense flat push icon="refresh" @click="getReportHistory" />Report
        History
        <q-space />
              ...
              <q-list dense style="min-width: 200px">
                ...
                <q-item
                  ...
                  @click="
                    openReportHistory(
                      props.row.id,
                      props.row.report_template_type !== 'plaintext' ? 'html' : 'plaintext',
                    )
                  "
                >
                  <q-item-section side>
                    <q-icon
                      :name="
                        props.row.report_template_type !== 'plaintext' ? 'code' : 'description'
                      "
                    />
                  </q-item-section>
                  <q-item-section
                    >Open
                    {{
                      props.row.report_template_type !== "plaintext" ? "HTML" : "Text"
                    }}
                ...
                <q-item
                  ...
                  :disabled="!!props.row.error_data"
                  @click="
                    downloadReportHistory(props.row.id, props.row.report_template_name, 'pdf')
                  "
                >
                  <q-item-section side>
                ...
                <q-item v-close-popup clickable @click="deleteHistory(props.row)">
                  <q-item-section side>
                    <q-icon name="delete" />
                  </q-item-section>
...
import { formatDate, capitalize } from "src/utils/format";
